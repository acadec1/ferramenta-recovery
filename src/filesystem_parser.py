...
def _partitions(img) -> list[dict]:
    ...
    partitions = []
    for part in volume:
        if not part.flags & tsk.TSK_VS_PART_FLAG_ALLOC:
            continue
...
def _is_deleted(entry) -> bool:
    ...
    if name is not None and name.flags & tsk.TSK_FS_NAME_FLAG_UNALLOC:
        return True
    meta = entry.info.meta
    if meta is not None and meta.flags & tsk.TSK_FS_META_FLAG_UNALLOC:
        return True
    return False
...
def _is_directory(entry) -> bool:
    tsk = _require_pytsk3()
    meta = entry.info.meta
    return meta is not None and meta.type == tsk.TSK_FS_META_TYPE_DIR


def _data_runs(entry) -> tuple[list[dict], bool]:
    """Runs de clusters do fluxo de dados principal e indicador de residencia."""
    tsk = _require_pytsk3()
    data_types = (tsk.TSK_FS_ATTR_TYPE_DEFAULT, tsk.TSK_FS_ATTR_TYPE_NTFS_DATA)
    ignored_run_flags = tsk.TSK_FS_ATTR_RUN_FLAG_FILLER | tsk.TSK_FS_ATTR_RUN_FLAG_SPARSE
    runs: list[dict] = []
    resident = False
    try:
    ...
    for attribute in attributes:
        ...
        if _decode_name(info.name):  # fluxos alternativos (ADS) sao ignorados
            continue
        if info.flags & tsk.TSK_FS_ATTR_FLAG_RES:
            resident = True
            continue
        try:
...
def scan_deleted_entries(device_path: str) -> list[dict]:
    ...
    """
    tsk = _require_pytsk3()
    image = tsk.Img_Info(device_path)
    entries: list[dict] = []
